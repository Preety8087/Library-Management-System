...
exports.home = async (req, res) => {
  try {
    ...
    console.log(err)
    // render with empty arrays to avoid redirect loop
    res.render('index', { popularBooks: [], recentBooks: [], msg: req.flash('msg') })
  }
}
...
exports.allBooks = (req, res) => {
  ...
  Book.find()
    ...
    .catch(err => {
      console.log(err)
      // render with empty data to avoid redirect loop
      res.render('customer/books', {
        books: [],
        msg: req.flash('msg'),
        currentPage: parseInt(currentPage),
...
exports.searchBook = (req, res) => {
  Book.find({ title: { $regex: req.query.title || '', $options: 'i' } })
    ...
    .catch(err => {
      console.log(err)
      // render with empty books to avoid redirect loop
      res.render('customer/search-book', { books: [], msg: req.flash('msg') })
    })
}
...
exports.booksByGenre = async  (req, res) => {
  try {
    ...
    console.log(err)
    // render with empty books to avoid redirect loop
    res.render('customer/books-genre', { books: [], genreName: '', genreParam: '', msg: req.flash('msg') })
  }
}
...
exports.updateProfile = async (req, res) => {
  ...
  if(!errors.isEmpty()) {
    try {
      ...
      console.log(err)
      // render with empty user to avoid redirect loop
      res.render('customer/profile-edit', {
        errors: errors.array(),
        user: null,
      })
...
exports.cart = (req, res) => {
  Cart.find().populate('user').populate('book')
    ...
    .catch(err => {
      console.log(err)
      // render with empty cart to avoid redirect loop
      res.render('customer/cart', { cartItems: [], msg: req.flash('msg') })
    })
}
    ...
    Cart.find({ user: user_id, book: book_id })
      .then(result => {
        if(inInventory.length !== 0) {
          ...
          .catch(err => {
            console.log(err)
            // redirect to home to avoid redirect loop
            res.redirect('/')
          })
        }
      })
...
exports.postBorrow = async (req, res) => {
  ...
  if(!errors.isEmpty()) {
    ...
    try {
      ...
      res.redirect(`/inventory/${user_id}?success=true`)
    } catch(err) {
      console.log(err)
      // redirect to home to avoid redirect loop
      res.redirect('/')
...
exports.borrowedBooks = async (req, res) => {
  try {
    ...
    BorrowHistory.find({ status: "Returned", book_returned: false })
      ...
      .then(borrowedBook => {
        res.render('customer/inventory', { url: req.params.id, borrowedBook, msg: req.flash('msg'), success: req.query.success === 'true' })
      })
      .catch(err => {
        console.log(err)
        // render with empty borrowed books to avoid redirect loop
        res.render('customer/inventory', { url: req.params.id, borrowedBook: [], msg: req.flash('msg'), success: false })
      })
  } catch(err) {
    console.log(err)
    // render with empty borrowed books to avoid redirect loop
...
exports.borrowHistory = async (req, res) => {
  try {
    ...
    console.log(err)
    // render with empty history to avoid redirect loop
    res.render('customer/borrow-history', { url: req.params.id, borrowHistory: [] })
  }
}
